...
- **Prompts**: Todos los prompts LLM están en `src/prompts.py`. Nunca hardcodear prompts en los servicios.
- **Señales de dominio**: Agregar nuevas señales únicamente en `src/nlp/domain_taxonomy.py`.
- **Esquema SQL**: No modificar sin migración. Usar Alembic o `bootstrap_db.py`.
- **Chunking**: Usar `semantic_chunk()` de `text_normalizer.py`, nunca `simple_chunk()`.
